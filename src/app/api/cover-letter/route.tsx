import { NextResponse } from 'next/server'
import { z } from 'zod'
import { generateCoverLetter } from '@/lib/coverLetter/generator'
import { renderToBuffer } from '@react-pdf/renderer'
import { CoverLetterPDF } from '@/lib/export/coverLetterPdf'
import type { ParsedCV } from '@/types'

const requestSchema = z.object({
  cv: z.object({
    personal: z.unknown(),
    summary: z.string(),
    experience: z.array(z.unknown()),
    skills: z.array(z.unknown()),
    projects: z.array(z.unknown()),
    targetRole: z.string(),
  }),
  jobTitle: z.string().optional(),
  companyName: z.string().optional(),
  hiringManagerName: z.string().optional(),
  jobDescription: z.string().optional(),
})

export async function POST(req: Request) {
  try {
    const body = await req.json()
    const parsed = requestSchema.safeParse(body)

    if (!parsed.success) {
      return NextResponse.json({ error: 'Invalid input' }, { status: 400 })
    }

    const { cv, jobTitle, companyName, hiringManagerName, jobDescription } = parsed.data

    const parsedCV: ParsedCV = {
      id: 'temp',
      userId: null,
      targetRole: cv.targetRole,
      status: 'draft',
      personal: cv.personal as ParsedCV['personal'],
      summary: cv.summary,
      experience: cv.experience as ParsedCV['experience'],
      education: [],
      skills: cv.skills as ParsedCV['skills'],
      projects: cv.projects as ParsedCV['projects'],
      atsScore: 0,
      atsKeywords: [],
      atsMissingKeywords: [],
      lastExportedAt: null,
      exportFormat: null,
      shareSlug: null,
      createdAt: new Date(),
      updatedAt: new Date(),
    }

    // Generate cover letter sections
    const sections = generateCoverLetter({
      cv: parsedCV,
      jobTitle,
      companyName,
      hiringManagerName,
      jobDescription,
    })

    // If format=pdf, return PDF
    const format = new URL(req.url).searchParams.get('format')
    if (format === 'pdf') {
      const pdfBuffer = await renderToBuffer(
        <CoverLetterPDF
          sections={sections}
          applicantName={parsedCV.personal?.name || 'Applicant'}
          applicantEmail={parsedCV.personal?.email || ''}
          companyName={companyName}
          date={new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
        />
      )

      return new NextResponse(new Uint8Array(pdfBuffer), {
        headers: {
          'Content-Type': 'application/pdf',
          'Content-Disposition': 'attachment; filename="Cover_Letter.pdf"',
        },
      })
    }

    // Default: return sections as JSON
    return NextResponse.json({ sections })

  } catch (error) {
    console.error('Cover letter error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
